<template>
  <div class="h-16 bg-zinc-900 px-4 rounded-xl flex items-center  gap-3">
    <div class="flex items-center gap-2 flex-1">
      ...
      <input v-bind:value="destination" :disabled="props.isGuestsInputOpen" type="text" placeholder="Para onde você vai?"
        class="bg-transparent text-lg placeholder-zinc-400 outline-none flex-1" />
    </div>

    <div class="flex items-center gap-2">
      <Calendar class="size-5 text-zinc-400" />
      <input v-bind:value="date" :disabled="props.isGuestsInputOpen" type="text" placeholder="Quando?"
